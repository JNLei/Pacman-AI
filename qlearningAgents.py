...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        TD_error = reward + self.discount * self.computeValueFromQValues(nextState)-qValue
        qValue += self.alpha*TD_error
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        dirrerence = reward + self.discount*self.computeValueFromQValues(nextState) - self.getQValue(state, action)
        features = self.featExtractor.getFeatures(state, action)
        for f in features.keys():
            self.weights[f] += self.alpha * dirrerence*features[f]
